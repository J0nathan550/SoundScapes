...
public:
  AudioPlayer::AudioPlayer(std::string idx, flutter::BinaryMessenger* messenger) {
    ...
    // WinRT's MediaPlayer auto-registers its own System Media Transport
    // Controls session as soon as it plays anything. audio_service_win
    // manages a separate, properly-branded SMTC session for the whole app,
    // so leaving this on makes Windows' Now Playing flyout show two
    // entries for the same app (this unbranded one, plus the real one).
