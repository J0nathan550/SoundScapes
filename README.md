...
SoundScapes is a Flutter application for searching, streaming, and downloading audio from YouTube. It plays audio-only in the background with full media controls, keeps a local library of liked songs, folders, and downloaded tracks, and runs as a native app on Android, Windows, and Linux.
...
- **Downloads for offline listening**: tracks are pulled down with a `yt-dlp` integration, with per-track progress reporting — bundled on Android, fetched on first use on Windows and Linux.
