...
class SettingsService {
  ...
  // Key string kept as-is (pre-dates Linux support) so existing Windows
  // users' persisted volume isn't dropped by a rename.
  static const _desktopVolumeKey = 'settings.windowsVolume';
  ...
  /// Desktop only (Windows, Linux) — there's no hardware volume control on
  /// desktop, so the app-level volume set via the Now Playing screen's
  /// slider is persisted here and restored on launch.
  double get desktopVolume => _prefs.getDouble(_desktopVolumeKey) ?? 1.0;

  Future<void> setDesktopVolume(double value) =>
      _prefs.setDouble(_desktopVolumeKey, value);
}
